package org.sunyata.game.edy.ai.core.trusteeship;

import org.sunyata.game.edy.ai.core.util.complex.*;

import java.util.*;
import java.util.Map.Entry;

public class TrusteeshipAI {
	private static TrusteeshipAI instance = new TrusteeshipAI();

	public static TrusteeshipAI getInstance() {
		return instance;
	}

	private TrusteeshipAI() {
	}
	...
	private Hand farmerPlayLord1Cards(HandMapSummary[] summarys, GameStatusAdv status, Hand curHand, int lowestCon) {
		...
		byte[] otherCardbyte = otherCards.getCards();
		int max = 0;
	...
	private void calcHandMap(GameStatusAdv status, HandMapSummary summary, int lowestControl, int pos) {
			...
			if (hands != null && hands.size() > 0) {
				...
				for (Hand hand : hands) {
					if (AIUtils.isBomb(hand)) {
						...
						if (countHandControl(hand, lowestControl) >= 2) {
							summary.changeMultiControlsHands(1);
						}
					}
				}
			}
		...
		summary.setEffectiveHands(summary.getUnHighestHandList().size() + summary.getMultiControlsHands()
				- summary.getSoloConNum() - summary.getExtraBomb());
	}
	...
	private Hand lordPlayFree(HandMapSummary[] summarys, GameStatusAdv status, int lowestCon) {
		...
		if (lordSummary.getRealHands() == 1) {
			...
			for (Hand highestHand : highestHands) {
				if(!AIUtils.isBomb(highestHand)){
					for (Hand unHighestHand : unhighestHands){
						if(unHighestHand.getType()==highestHand.getType()&&unHighestHand.getLen()==highestHand.getLen()){
							if (unHighestHand != null) {
								hand = unHighestHand;
								break;
							}	
						}		
			...
			int farmerSolo = -1;
			if (upCards.getNum() == 1||downCards.getNum() == 1) {
				farmerSolo = upCards.getHighest().ordinal();
				if(farmerSolo<downCards.getHighest().ordinal()){
					farmerSolo = downCards.getHighest().ordinal();
				}
			}			
			if (farmerSolo >= 0) {
				for (Hand highestHand : highestHands) {
