/*
 * Copyright 2017 AGTech.com All right reserved. This software is the
 * confidential and proprietary information of AGTech.com ("Confidential
 * Information"). You shall not disclose such Confidential Information and shall
 * use it only in accordance with the terms of the license agreement you entered
 * into with AGTech.com.
 */

package org.sunyata.lottery.edy.common.vo;

/**
 * （到互动引擎）获取符号的Response
 *
 * @author Zhanglei
 */
public class GetSymbolResponse extends GamePlayResponse {

    private static final long serialVersionUID = 783466695038715531L;

    @Override
    public String toString() {
        return "GetSymbolResponse{" + super.toString() + '}';
    }
}
